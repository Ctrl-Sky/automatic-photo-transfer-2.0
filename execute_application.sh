...
TIMEZONE=$(yq "$YAML_ROOT.timezone" $YAML_FILE)
...
python3.12 src/main.py --device "$DEVICE" --path_to_photos "$PATH_TO_PHOTOS" --destination_path "$DESTINATION" --timezone "$TIMEZONE" --migration_name "$MIGRATION" --end_on "$END_ON"